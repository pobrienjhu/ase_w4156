...
public class InputValidatorTruncator {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(InputValidatorTruncator.class);
	 ...
	 *             the exception
	 */
	private void handleFieldsForClass(Object object, Class<?> aClass)
			throws Exception {
		...
		for (Field field : declaredFields) {
			...
			boolean truncateFields = field
					.isAnnotationPresent(TruncateFields.class);
			if (truncateFields) {
	...
	private void handleSubObject(Object object, Field field) throws Exception {
		...
		try {
			...
			logger.error(" (" + (object != null ? object.toString() : "null")
					+ ",field=" + (field != null ? field.getName() : "null")
					+ ") " + "Exception caught");
			throw new Exception("Problem truncating field(" + field.getName()
					+ ")", e);
		}
	}
	 ...
	 *             the exception
	 */
	private void handleTruncateFields(Object object, Field field)
			throws Exception {
		...
		try {
			...
			throw new Exception("Problem truncating field(" + fieldName + ")",
					e);
		}
	...
	private void handleStringField(Object object, Field field) throws Exception {
		...
		if (column != null) {
			String fieldName = field.getName();
			try {
				String value = (String) PropertyUtils.getProperty(object,
						fieldName);
				if (value != null) {
					int allowedLength = column.value();
					boolean valueIsTooLong = allowedLength > 0
	 ...
	 */
	private void truncateValue(Object object, Field field, String value,
			int allowedLength) throws InvocationTargetException,
