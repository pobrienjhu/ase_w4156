...
public class EventValidationHelper {
	 ...
	 */
	public static void validateEventUpdate(Event event)
			throws ValidationException {
		...
		if (event.getStartTime() != null
				&& DateTime.now().isAfter(event.getStartTime())) {
			validationException
					.addMessage("Can not update event after start time");
		}
	 ...
	 *             the validation exception
	 */
	public static void validateEventCreation(Event event)
			throws ValidationException {
		...
		if (event.getStartTime() != null && event.getEndTime() != null
			...
			validationException
					.addMessage("Start date must be before end date.");
		}

		if (event.getVoteCategories() == null
				|| event.getVoteCategories().isEmpty()) {
			validationException
					.addMessage("At least one voting category must be supplied.");
		} else {
	 ...
	 *             the validation exception
	 */
	public static void validateVoteCategory(VoteCategory category)
			throws ValidationException {
		...
		if (StringUtils.isEmpty(StringUtils.trimToEmpty(category
				.getCategoryName()))) {
			validationException.addMessage("Vote Category must have a name.");
		}

		if (category.getVoteOptions() == null
				|| category.getVoteOptions().size() < 2) {
			validationException
					.addMessage("Vote Category must have at least two options.");
		}
