...
public class EventTypeAdapterTest {
	...
	public void testSerializeAndDeserializeEvent() {
		...
		String json = gson.toJson(event).replace("[],",
	...
	public void testSerializeAndDeserializeVoteCategories() {
		...
		VoteCategory secondCategory = new VoteCategory(event,
				"Second Category", "First Category Description");
		...
		event.setVoteCategories(ImmutableList.<VoteCategory> of(firstCategory,
				secondCategory));
		...
		assertEquals(Long.valueOf(456L), deserialized.getVoteCategories()
				.get(0).getId());
		assertEquals(3, deserialized.getVoteCategories().get(0)
				.getVoteOptions().size());
		assertEquals(Long.valueOf(4561L),
				deserialized.getVoteCategories().get(0).getVoteOptions().get(0)
						.getId());
		assertEquals(Long.valueOf(4562L),
				deserialized.getVoteCategories().get(0).getVoteOptions().get(1)
						.getId());
		assertEquals(Long.valueOf(4563L),
				deserialized.getVoteCategories().get(0).getVoteOptions().get(2)
						.getId());

		assertEquals(Long.valueOf(789L), deserialized.getVoteCategories()
				.get(1).getId());
		assertEquals(2, deserialized.getVoteCategories().get(1)
				.getVoteOptions().size());
		assertEquals(Long.valueOf(7891L),
				deserialized.getVoteCategories().get(1).getVoteOptions().get(0)
						.getId());
		assertEquals(Long.valueOf(7892L),
				deserialized.getVoteCategories().get(1).getVoteOptions().get(1)
						.getId());
