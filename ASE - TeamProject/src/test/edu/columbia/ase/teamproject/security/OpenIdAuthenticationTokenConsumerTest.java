...
public class OpenIdAuthenticationTokenConsumerTest {
    ...
    public void setUp() {
        ...
        this.tokenConsumer = new OpenIdAuthenticationTokenConsumer(mockAccountDao);
    }

    @Test
    public void testUnsuccessfulAuthenticationThrowsException() {
        OpenIDAuthenticationStatus[] statuses = OpenIDAuthenticationStatus.values();
    ...
    public void testFirstUserGetsAdminPrivileges() {
        when(mockToken.getStatus()).thenReturn(OpenIDAuthenticationStatus.SUCCESS);
        when(mockToken.getIdentityUrl()).thenReturn("http://localhost");
        when(mockToken.getAttributes()).thenReturn(ImmutableList.of(mockAttribute));
        when(mockAttribute.getName()).thenReturn("email");
        when(mockAttribute.getValues()).thenReturn(ImmutableList.of("user@example.com"));
        when(mockAccountDao.getNumberOfUsers()).thenReturn(0L);
        UserDetails details = tokenConsumer.loadUserDetails(mockToken);
        GrantedAuthority expected = AuthorityUtils.createAuthorityList("ADMIN").get(0);
    ...
    public void testSecondUserDoesNotGetAdminPrivileges() {
        ...
        UserDetails details = tokenConsumer.loadUserDetails(mockToken);
        GrantedAuthority expected = AuthorityUtils.createAuthorityList("USER").get(0);
        GrantedAuthority unexpected = AuthorityUtils.createAuthorityList("ADMIN").get(0);
